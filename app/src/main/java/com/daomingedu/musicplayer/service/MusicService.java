package com.daomingedu.musicplayer.service;
...
public class MusicService extends Service {
    ...
    private void addPlayListInner(Music music) {
        if (!playingMusicList.contains(music)) {
            ...
            PlayingMusic playingMusic = new PlayingMusic(music.songUrl, music.title, music.categoryName, music.number);
            playingMusic.save();
        }
        currentMusic = music;
        isNeedReload = true;
    ...
    private void addPlayListInner(List<Music> musicList) {
        ...
        for (Music i : musicList) {
            PlayingMusic playingMusic = new PlayingMusic(i.songUrl, i.title, i.categoryName, i.number);
            playingMusic.save();
        }
    ...
    private void playMusicItem(Music item, boolean reload) {
        ...
        if (item == null) {
            ...
            File file = new File(item.songUrl);
            if (!file.exists()) {
                if (playingMusicList.contains(item)) {
                }
                removeMusicInner(playingMusicList.indexOf(item));
    ...
    private void initPlayList() {
        ...
        for (PlayingMusic i : list) {
            Music m = new Music(i.songUrl, i.title, i.categoryName, i.number);
